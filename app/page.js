...
//   const [savedData, setSavedData] = useState([]); // Store multiple datasets
...
//       // Convert the sheet data into an array of flashcards
//       const cards = jsonData.map((row) => ({
//         word: row[0],
//         meaning: row[1],
//       }));

//       // Save the flashcards data in local state and reset index
//       setFlashcards(shuffleArray(cards));
//       setCurrentIndex(0); // Reset index when new file is uploaded

//       // Save this file data in localStorage and update the savedData state
//       const newFileData = {
//         name: file.name,
//         cards,
//       };
...
//       setSelectedFile(file.name); // Set this file as the current selection
//     };
...
//     setCurrentIndex(0); // Reset the index to 0 when switching between files
//     setIsFlipped(false); // Ensure the card is not flipped when switching files
